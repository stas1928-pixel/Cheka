...
js/repertoire.js        loads the generated data and overlays hand notes
js/repertoire.data.js   GENERATED by tools/verify-repertoire.mjs from tools/repertoire.seed.mjs — never hand-edit
js/notes.js             hand-written explanations per line id (survives regeneration)
...
1. Edit `tools/repertoire.seed.mjs` (add a line with a name, moves and sources).
2. Verify and regenerate the data file:

```bash
node tools/verify-repertoire.mjs 18 > js/repertoire.data.js
```

3. Read the report on stderr: fix every DROP / SIDE? / REPEAT, re-run.
4. Add a note for the new line id in `js/notes.js`, run `npm test`, commit.
